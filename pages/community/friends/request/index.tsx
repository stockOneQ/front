...
const ReqFriendsPage = () => {
  ...
  >([]);
  const [reqFriendsList, setReqFriendsList] = useState<
    FriendsListType['friendsList']
  >([]);
  useEffect(() => {
    const getData = async () => {
      ...
      while (true) {
        ...
        try {
          ...
          setWaitingFriendsList(prev => [...prev, ...waitingFriendsData]);
          waiting_offset =
            waitingFriendsData[waitingFriendsDataLen - 1]?.id || -1; // 마지막으로 데이터 넘어온 친구의 id
      ...
      while (true) {
        ...
        try {
          ...
          setReqFriendsList(prev => [...prev, ...reqFriendsData]);
